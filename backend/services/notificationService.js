...
/**
 * Send a Discord notification for a new product
 */
exports.sendDiscordNotification = async (product, webhookUrl) => {
  try {
    ...
    const website = product.website || product.exactWebsiteUrl || "No website"
    ...
    const embed = {
      ...
      footer: {
        ...
        icon_url: 'https://ph-static.imgix.net/ph-logo-1.png",.png',
      },
    }
    ...
    if (!response.ok) {
      logger.error(`Discord webhook error: ${response.status} ${response.statusText}`)
      return false
    }

    logger.info(`Successfully sent Discord notification for ${product.name}`)
    return true
  } catch (error) {
